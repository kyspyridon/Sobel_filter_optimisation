...
int convolution2D_hor(int posy, int posx, const unsigned char *input) {
	...
	return (input[(posy - 1)*SIZE + posx - 1] * hor_operator[0][0]
				+ input[(posy - 1)*SIZE + posx + 1] * hor_operator[0][2]
				+ input[(posy)*SIZE + posx - 1] * hor_operator[1][0]
				+ input[(posy)*SIZE + posx + 1] * hor_operator[1][2]
				+ input[(posy + 1)*SIZE + posx - 1] * hor_operator[2][0]
				+ input[(posy + 1)*SIZE + posx + 1] * hor_operator[2][2]);
}

int convolution2D_ver(int posy, int posx, const unsigned char *input) {

	return (input[(posy - 1)*SIZE + posx - 1] * ver_operator[0][0]
				+ input[(posy - 1)*SIZE + posx] * ver_operator[0][1]
				+ input[(posy - 1)*SIZE + posx + 1] * ver_operator[0][2]
				+ input[(posy + 1)*SIZE + posx - 1] * ver_operator[2][0]
				+ input[(posy + 1)*SIZE + posx] * ver_operator[2][1]
				+ input[(posy + 1)*SIZE + posx + 1] * ver_operator[2][2]);
}
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			conv_res_hor = convolution2D_hor(i, j, input);
			conv_res_ver = convolution2D_ver(i, j, input);
