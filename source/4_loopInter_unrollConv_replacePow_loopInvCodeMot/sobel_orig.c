 ...
 * pixel we process.														  */
int convolution2D_hor(int posy, int posx, const unsigned char *input) {
	const int posy_min1 = (posy - 1) * SIZE;
	const int posy_plus1 = (posy + 1) * SIZE;
	const int posy_size = posy * SIZE;
	const int	posx_min1 = (posx - 1);
	const int posx_plus1 = (posx + 1);

		return (input[posy_min1 + posx_min1] * hor_operator[0][0]
...
int convolution2D_ver(int posy, int posx, const unsigned char *input) {
	...
	return (input[posy_min1 + posx_min1] * ver_operator[0][0]
